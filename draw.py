...
def highlight(screen, click_pos):
    if click_pos != None: pygame.draw.rect(screen, const.BLUE, (click_pos[1] * const.SQUARE_SIDE, click_pos[0] * const.SQUARE_SIDE, const.SQUARE_SIDE, const.SQUARE_SIDE), math.ceil(const.SQUARE_SIDE / 30))

# main drawing function
def draw_main(screen, click_pos):
    screen.fill(const.BLACK)
    draw_board(screen)
    draw_pawns(screen)
    highlight(screen, click_pos)
    pygame.display.flip()
