...
def manage_move(old_pos, new_pos):
    ...
    if find_pawn(old_pos).get_type() == 0:
        ...
        if find_pawn(old_pos).get_type() == 0 and (find_pawn(old_pos).get_color() == 0 and old_pos[0] >= new_pos[0]) or (find_pawn(old_pos).get_color() == 1 and old_pos[0] <= new_pos[0]): return False
    ...
    if abs(old_pos[0] - new_pos[0]) == 2 and abs(old_pos[1] - new_pos[1]) == 2:
        ...
        if find_pawn(mid_pos) == None or find_pawn(old_pos).get_color() == find_pawn(mid_pos).get_color(): return False
        # deleting captured pawn
        find_pawn(mid_pos).delete()
    ...
    if (find_pawn(new_pos).get_color() == 0 and new_pos[0] == 7) or (find_pawn(new_pos).get_color() == 1 and new_pos[0] == 0): find_pawn(new_pos).upgrade()
    ## correct move
    return True
